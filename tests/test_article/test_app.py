...
def test_get_article(client):
    ...
    """
    article = Article(
        author="jane@doe.com", title="New Article", content="Super extra awesome article"
    ).save()
    response = client.get(
...
@pytest.mark.parametrize(
    "data",
    [
        {"author": "John Doe", "title": "New Article", "content": "Some extra awesome content"},
        {"author": "John Doe", "title": "New Article"},
...
def test_create_list_get(client):
    requests.post(
        ...
        json={"author": "john@doe.com", "title": "New Article", "content": "Some extra awesome content"},
    )
    response = requests.get("http://localhost:5000/article-list/")
    articles = response.json()
    response = requests.get(f"http://localhost:5000/article/{articles[0]['id']}/")
